	...
	public void mesclarPedido(Pedido outroPedido) {
        ...
        for (int i = 0; i < outroPedido.quantProdutos; i++) {
            ...
            for (int j = 0; j < this.quantProdutos; j++) {
                if (this.itens[j].equals(itemOutro)) {
                    existe = true;
                    break;
                }
            }
        ...
        if (this.quantProdutos + novosItens > MAX_PRODUTOS) {
            throw new IllegalStateException("Capacidade máxima do pedido excedida ao mesclar os carrinhos.");
        }
        ...
        for (int i = 0; i < outroPedido.quantProdutos; i++) {
            ...
            for (int j = 0; j < this.quantProdutos; j++) {
                if (this.itens[j].equals(itemOutro)) {
                    this.itens[j].setQuantidade(this.itens[j].getQuantidade() + itemOutro.getQuantidade());
                    
                    if (itemOutro.getPrecoVenda() < this.itens[j].getPrecoVenda()) {
                        this.itens[j].setPrecoVenda(itemOutro.getPrecoVenda());
                    }
                    encontrou = true;
                    break;
                }
            ...
            if (!encontrou) {
                this.itens[quantProdutos] = new ItemDePedido(
                    itemOutro.getProduto(), 
                    itemOutro.getQuantidade(), 
                    itemOutro.getPrecoVenda()
                );
                this.quantProdutos++;
            }
        }
        ...
        for (int i = 0; i < quantProdutos; i++) {
            ...
            if (item != null) {
                ...
                if (qtd > 10) {
                    ...
                    System.out.printf("%s | Qtd: %d | Preço: R$%.2f | Subtotal: R$%.2f (5%% OFF aplicado)%n", nome, qtd, precoUnitario, subtotal);
                } else {
                    System.out.printf("%s | Qtd: %d | Preço: R$%.2f | Subtotal: R$%.2f%n", nome, qtd, precoUnitario, subtotal);
                }
